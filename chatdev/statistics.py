...
def prompt_cost(
    ...
    return (
        num_prompt_tokens * input_cost_map[model_type] / 1000.0
...
def get_info(dir, log_filepath):
    ...
    if os.path.exists(dir):
        ...
        if "meta.txt" in filenames:
            lines = (
                open(os.path.join(dir, "meta.txt"), encoding="utf8").read().split("\n")
            )
            version_updates = (
                float(
                    [
        ...
        if "requirements.txt" in filenames:
            lines = (
                open(os.path.join(dir, "requirements.txt"), encoding="utf8")
                .read()
                .split("\n")
            )
        ...
        if "manual.md" in filenames:
            lines = (
                open(os.path.join(dir, "manual.md"), encoding="utf8").read().split("\n")
            )
        ...
        for filename in filenames:
            if filename.endswith(".py"):
                ...
                lines = (
                    open(os.path.join(dir, filename), encoding="utf8")
                    .read()
                    .split("\n")
                )
        ...
        lines = open(log_filepath, encoding="utf8").read().split("\n")
        sublines = [line for line in lines if "| **model_type** |" in line]
        if len(sublines) > 0:
            model_type = (
                sublines[0].split("| **model_type** | ModelType.")[-1].split(" | ")[0]
            )
